...
/*
 * A ageingSpeed-improved simplex noise algorithm for 2D, 3D and 4D in Java.
 *
...
public class SimplexNoise {  // Simplex noise in 2D, 3D and 4D
    ...
    public SimplexNoise(double scale) {
        this.scale = scale;
    }
    ...
    public void generateNoise(int size) {
        noiseMap = new double [size][size];
        noiseGenerated = true;
        offset += 0.01;

        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                noiseMap[i][j] = noise((double)(i)/size * scale, (double)(j)/size * scale, offset);
            }
        }
    }
}
